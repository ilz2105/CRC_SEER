...
```{r load.continuous.data, echo=FALSE,  message=FALSE}
...
chol_mg = diet_chol %>% 
  ggplot(aes(x = period, y = (aggregate(diet[,5], list(diet$period), mean))))+
   geom_line(aes(color = AGE_GRP)) +
  facet_wrap(~race+gender)+
   labs(
    title = "cholesterol_mg intake from 1975-2014",
    x = "Periods (5-year groups)",
    y = "Intake of cholesterol (mg)",
    caption = "by 5-year age groups (25-49)"
  )
#chol_mg
...
alc_mg = alc%>% 
  ggplot(aes(x = period, y = alc_pmo))+
  stat_summary(fun.y="mean", geom="bar")+
   geom_line(aes(color = gender)) +
  facet_wrap(~AGE_GRP)+
   labs(
    title = "alcohol per month intake from 1975-2014",
    x = "Periods (5-year groups)",
    y = "Intake of cholesterol (mg)",
    caption = "by 5-year age groups (25-49)"
  )
...
#ggplot(alc, aes(x=period, y=alc_pmo)) + stat_summary(fun.y="mean", geom="bar")+
 # facet_grid(~race+gender)+
#  scale_colour_manual(values = AGE_GRP)
```
